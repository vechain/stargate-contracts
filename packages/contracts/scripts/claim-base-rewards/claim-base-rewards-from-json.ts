...
async function main() {
    ...
    const inputPath = path.join(__dirname, `/data/base-rewards-data-${config.environment}.json`);
    if (!fs.existsSync(inputPath)) {
        throw new Error(`Input file not found: ${inputPath}`);
    }
    const inputData: BaseRewardsData = JSON.parse(fs.readFileSync(inputPath, "utf8"));
    ...
    const outputData: BaseRewardsExecutionReport = {
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
        ...
        for (const reward of batchRewards) {
            ...
            const currentClaimableBaseVthoRewards =
                await stargateNFTContract.claimableVetGeneratedVtho(reward.tokenId);
            if (currentClaimableBaseVthoRewards.toString() !== reward.claimableRewards) {
                outputData.results.push({
                    tokenId: reward.tokenId,
                    owner: reward.owner,
                    status: "error",
                    error: "Claimable base rewards mismatch",
                });
                outputData.failedRewardsClaimed++;
                continue;
            }
        ...
        const stargateNFTClaimRewardsFunction = ABIContract.ofAbi(
            StargateNFTV2__factory.abi
        ).getFunction("claimVetGeneratedVtho") as any;
        const clauses: Clause[] = [];
        for (const reward of validRewards) {
            // Claim base VTHO rewards
            clauses.push(
                Clause.callFunction(stargateNFTAddress, stargateNFTClaimRewardsFunction, [
                    reward.tokenId,
                ])
            );
        }
    ...
    log(`\n📊 Base rewards claiming process completed!\n`);
    ...
    const outputFileName = executeTransactions ? "base-rewards-results" : "base-rewards-dryrun";
