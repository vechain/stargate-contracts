...
The aim of this script is to claim all the pending rewards for all the tokens in the StargateDelegationV3 contracts before the hayabusa upgrade.
...
This command is not available for devnet because it has no relevant information in the StargateDelegationV3 contracts.

### Get rewards data

```bash
yarn contracts:get-claim-delegation-rewards-data:solo # or testnet, mainnet
```
...
```bash
yarn contracts:claim-delegation-rewards:solo # or testnet, mainnet
...
The output is saved in the `data` folder in the `packages/contracts/scripts/claim-delegation-rewards` directory.

The output is saved in the `results` folder in the `packages/contracts/scripts/claim-delegation-rewards` directory.
