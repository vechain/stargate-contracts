...
describe("StargateNFT VET Generated VTHO Rewards", () => {
  it("should be able to claim generated VTHO rewards generated by the protocol", async () => {
    ...
    const tx = await stargateNFTContract.connect(user).claimVetGeneratedVtho(tokenId);
    await tx.wait();
  ...
  it("when the VTHO generation is stopped, the rewards are calculated based on the end timestamp", async () => {
    const config = createLocalConfig();
    config.DELEGATION_PERIOD_DURATION = 3; // 3 blocks
    config.TOKEN_LEVELS[1].level.maturityBlocks = 3;
    config.TOKEN_LEVELS[1].level.vetAmountRequiredToStake = ethers.parseEther("1");

    const { stargateNFTContract, deployer, otherAccounts, mockedVthoToken } =
  ...
  it("should be able to claim generated VTHO rewards for migrated NFTs", async () => {
    ...
    // User stakes and mints an NFT
    await stargateNFTContract.connect(user).migrate(legacyNodeId, {
    ...
    const timestampWhenUserStaked =
      await stargateNFTContract.getLastVetGeneratedVthoClaimTimestamp(legacyNodeId);

    // User claims his rewards
    const tx = await stargateNFTContract.connect(user).claimVetGeneratedVtho(legacyNodeId);
    await tx.wait();
    const txBlock = await tx.getBlock();
    if (!txBlock) {
      throw new Error("Transaction block not found");
  ...
  it("should automatically claim pending rewards when unstaking", async () => {
    ...
    const tx = await stargateNFTContract.connect(user).unstake(legacyNodeId);
    const txBlock = await tx.getBlock();
    if (!txBlock) {
      throw new Error("Transaction block not found");
    }
  ...
  it("Every time the NFT is transferred, the pending rewards are claimed", async () => {
    ...
    await mineBlocks(2);
    await stargateNFTContract.claimVetGeneratedVtho(legacyNodeId);
  ...
  it("Default admin should be able to correctly handle the hayabusa hardfork", async () => {
    ...
    await stargateNFTContract.connect(user1).migrate(legacyNodeId1, {
      value: level.vetAmountRequiredToStake,
    });

    // User2 migrates their NFT
    await stargateNFTContract.connect(user2).migrate(legacyNodeId2, {
      value: level.vetAmountRequiredToStake,
    });

    const user1StakeTimestamp =
      await stargateNFTContract.getLastVetGeneratedVthoClaimTimestamp(legacyNodeId1);
    const user2StakeTimestamp =
      await stargateNFTContract.getLastVetGeneratedVthoClaimTimestamp(legacyNodeId2);
    ...
    await stargateNFTContract.connect(deployer).setVthoGenerationEndTimestamp(hardforkTimestamp);
    ...
    await stargateNFTContract.connect(user1).claimVetGeneratedVtho(legacyNodeId1);
    expect(await mockedVthoToken.balanceOf(user1.address)).to.equal(expectedRewardsUser1);

    // User2 claims rewards
    await stargateNFTContract.connect(user2).claimVetGeneratedVtho(legacyNodeId2);
