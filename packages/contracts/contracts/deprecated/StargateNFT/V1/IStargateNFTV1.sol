...
import { DataTypesV1 } from "./libraries/DataTypesV1.sol";

interface IStargateNFTV1 is IERC721, IERC721Enumerable {
    // ------------------ Events ------------------ //
    ...
    function getLevel(uint8 _levelId) external view returns (DataTypesV1.Level memory);

    function getLevels() external view returns (DataTypesV1.Level[] memory);
    ...
    function getToken(uint256 _tokenId) external view returns (DataTypesV1.Token memory);

    function tokensOwnedBy(address _owner) external view returns (DataTypesV1.Token[] memory);
