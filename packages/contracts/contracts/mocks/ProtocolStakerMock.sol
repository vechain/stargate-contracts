...
import "../../contracts/interfaces/IProtocolStaker.sol";
...
contract ProtocolStakerMock is IProtocolStaker {
    ...
    function helper__setValidatorStatus(
        address _validator,
    ...
    function helper__setValidationCompletedPeriods(
        address _validator,
        uint32 _completedPeriods
    ) external {
    ...
    function signalExit(address _validator) external {
        ...
        validations[_validator].exitBlock =
            validation.startBlock + (validation.completedPeriods + 1) * validation.period;
    }
    ...
    function signalDelegationExit(uint256 _delegationID) external {
        ...
        delegations[_delegationID].endPeriod =
            validations[delegations[_delegationID].validator].completedPeriods + 1;
    }
        ...
        returns (uint32 period, uint32 startBlock, uint32 exitBlock, uint32 completedPeriods)
    {
        ...
        return (
            validation.period,
            validation.startBlock,
            validation.exitBlock,
            validation.completedPeriods
        );
    }
